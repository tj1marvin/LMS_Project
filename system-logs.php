...
// Define path to the system log file
// **IMPORTANT: You MUST adjust this path to the actual location of your PHP error log file on your server.**
// The location varies depending on your server setup and PHP configuration (php.ini).
// Common locations might include:
// - Linux/Unix: /var/log/apache2/error.log or /var/log/httpd/error_log or /var/log/php_errors.log
// - Windows (Apache): apache/logs/error.log
// - Windows (IIS): Check IIS logs or PHP error log location configured in php.ini
$logFilePath = '/Applications/XAMPP/xamppfiles/logs/php_error_log'; // **<--- ADJUST THIS PATH!**
...
if (file_exists($logFilePath) && is_readable($logFilePath)) {
    ...
    // Optional: Reverse lines to show most recent logs at the top (if log format is line-based)
    $logContent = implode("\n", array_reverse(explode("\n", $logContent)));
} else {
    $logReadError = true;
    error_log("Error: Could not read system log file at: " . $logFilePath); // Log error to server's error log if reading fails
}
            ...
            <div class="content">
                <div class="container-fluid">
                    <div class="log-container">
                        ...
                        <p class="mb-3">This page displays the raw system logs. It is useful for debugging and monitoring system errors.</p>

                        <?php if ($logReadError): ?>
                            <div class="alert alert-danger log-error" role="alert">
                                <strong>Error:</strong> Could not read system log file. Please check:
                                <ul>
                                    <li>If the log file exists at the specified path: <code><?php echo htmlspecialchars($logFilePath); ?></code></li>
                                    <li>If the web server user has read permissions to access this file.</li>
                                    <li>That the path in <code>system-logs.php</code> is correctly configured for your server environment.</li>
                                </ul>
                            </div>
                        <?php else: ?>
                            <div class="card log-display-card">
                                <div class="card-body">

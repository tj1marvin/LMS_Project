...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $bookYear = (int)$_POST['bookYear']; // Cast to integer
    $bookDescription = $_POST['bookDescription'];
    $availableCopies = (int)$_POST['availableCopies']; // New field for available copies

    // Handle image upload
    if (!empty($_FILES['bookImage']['name'])) {
        $imagePath = 'uploads/images/' . basename($_FILES['bookImage']['name']);
    ...
    if (!empty($_FILES['bookFile']['name'])) {
        $filePath = 'uploads/files/' . basename($_FILES['bookFile']['name']);
        move_uploaded_file($_FILES['bookFile']['tmp_name'], $filePath);
    } else {
    ...
    $sql = "UPDATE books SET title = ?, author = ?, isbn = ?, genre = ?, year = ?, description = ?, available_copies = ?, image_path = ?, file_path = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssisissi", $bookTitle, $bookAuthor, $bookISBN, $bookGenre, $bookYear, $bookDescription, $availableCopies, $imagePath, $filePath, $bookId);

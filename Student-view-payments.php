...
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'payment_date'; // Default sort by payment date
$order = isset($_GET['order']) ? $_GET['order'] : 'DESC'; // Default order descending

// Validate sort parameter
$allowedSortColumns = ['payment_id', 'amount', 'payment_date', 'payment_method', 'payment_type'];
if (!in_array($sort, $allowedSortColumns)) {
    $sort = 'payment_date'; // Default to payment_date if an invalid sort column is provided
}

// Validate order parameter
$order = strtoupper($order); // Convert to uppercase for consistency
if ($order != 'ASC' && $order != 'DESC') {
    $order = 'DESC'; // Default to DESC if an invalid order is provided
}

// SQL query to fetch payments for the logged-in student, joined with fines and penalties
$sql = "SELECT 
            p.payment_id, 
            p.amount, 
            p.payment_date, 
            ...
            pen.penalty_amount,
            CASE 
                WHEN f.fine_amount IS NOT NULL THEN 'Fine'
                WHEN pen.penalty_amount IS NOT NULL THEN 'Penalty'
                ELSE 'Payment' 
            END AS payment_type
        FROM payments p
        LEFT JOIN fines f ON p.payment_id = f.payment_id
        LEFT JOIN penalties pen ON p.payment_id = pen.payment_id
        WHERE p.student_id = ?
        ORDER BY $sort $order
...
$totalRecordsSql = "SELECT COUNT(*) FROM payments WHERE student_id = ?";
$totalRecordsStmt = $conn->prepare($totalRecordsSql);
$totalRecordsStmt->bind_param("i", $student_id);
                                ...
                                <?php if ($result->num_rows > 0): ?>
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover">
                                            <thead>
                                                <tr>
                                                    <th><a href="?page=<?php echo $page; ?>&sort=payment_id&order=<?php echo ($sort == 'payment_id' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Payment ID</a></th>
                                                    <th><a href="?page=<?php echo $page; ?>&sort=amount&order=<?php echo ($sort == 'amount' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Amount</a></th>
                                                    <th><a href="?page=<?php echo $page; ?>&sort=payment_date&order=<?php echo ($sort == 'payment_date' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Payment Date</a></th>
                                                    <th><a href="?page=<?php echo $page; ?>&sort=payment_method&order=<?php echo ($sort == 'payment_method' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Payment Method</a></th>
                                                    <th>Transaction ID</th>
                                                    <th><a href="?page=<?php echo $page; ?>&sort=payment_type&order=<?php echo ($sort == 'payment_type' && $order == 'ASC') ? 'DESC' : 'ASC'; ?>">Payment Type</a></th>
                                                </tr>
                                            </thead>
                                            <tbody>

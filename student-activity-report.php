...
/**
 * Function to Get Most Active Students by Book Issues (SECURE - Prepared Statement)
 * Fetches a list of students who have issued the most books.
 * @param mysqli $conn Database connection object.
 * @param int $limit Maximum number of students to retrieve (default 10).
 * @return array Array of active student data, or empty array on error.
 */
function getMostActiveStudentsReport($conn, $limit = 10)
{
    $sql = "SELECT
                sr.student_id,
                CONCAT(sr.first_name, ' ', sr.last_name) AS student_name,
                sr.email AS student_email,
                COUNT(ib.student_id) AS issue_count
            FROM students_registration sr
            LEFT JOIN issue_book ib ON sr.student_id = ib.student_id
            GROUP BY sr.student_id, student_name, student_email
            ORDER BY issue_count DESC
            LIMIT ?";

    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        error_log("getMostActiveStudentsReport Prepare Error: " . $conn->error);
        return [];
    }

    $stmt->bind_param("i", $limit);
    if (!$stmt->execute()) {
        error_log("getMostActiveStudentsReport Execute Error: " . $stmt->error);
        return [];
    }
...
/**
 * Function to Get Recent Student Book Issue/Return Activity (SECURE - Prepared Statement)
 * Fetches recent records of books issued or returned by students.
 * @param mysqli $conn Database connection object.
 * @param int $limit Maximum number of activity records to retrieve (default 10).
 * @return array Array of recent student activity data, or empty array on error.
 */
function getRecentStudentActivity($conn, $limit = 10)
{
    $sql = "SELECT
                ib.issue_id,
                ib.issue_date,
            ...
            JOIN students_registration sr ON ib.student_id = sr.student_id
            ORDER BY ib.issue_date DESC
            LIMIT ?";

    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        error_log("getRecentStudentActivity Prepare Error: " . $conn->error);
        return [];
    }

    $stmt->bind_param("i", $limit);
    if (!$stmt->execute()) {
        error_log("getRecentStudentActivity Execute Error: " . $stmt->error);
        return [];
    }
...
// --- Fetch Report Data ---
$activeStudentsReport = getMostActiveStudentsReport($conn);
$recentStudentActivity = getRecentStudentActivity($conn);
                ...
                <nav
                    ...
                    <div class="container-fluid">
                        <nav
                            ...
                            <h1 class="mb-4">Student Activity Report</h1>
                        </nav>
            ...
            <div class="content">
                <div class="container-fluid">
                    <div class="report-container">
                        ...
                        <p class="mb-3">This report provides an overview of student activity within the library system.</p>
                        ...
                        <div class="row">
                                ...
                                <div class="card report-card">
                                    <div class="card-body">
                                        ...
                                        <div class="card-text">
                                            ...
                                            <p class="text-muted"><small>Total count of registered students</small></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-8">
                                <div class="card report-card">
                                    <div class="card-body">
                                        <h5 class="card-title">Most Active Students (by Book Issues)</h5>
                                        <div class="table-responsive">
                                            <table class="table table-striped table-hover">
                                                ...
                                                <tbody>
                                                    <?php if (empty($activeStudentsReport)): ?>
                                                        <tr>
                                                            <td colspan="4">No student activity data available.</td>
                                                        </tr>
                                                    <?php else: ?>
                                                        <?php $rank = 1;
                                                        foreach ($activeStudentsReport as $student): ?>
                                                            <tr>
                                                                <td><?php echo $rank++; ?></td>
                                                                <td><?php echo htmlspecialchars($student['student_name']); ?></td>
                                                                <td><?php echo htmlspecialchars($student['student_email']); ?></td>
                                                                <td><?php echo htmlspecialchars($student['issue_count']); ?></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    <?php endif; ?>
                                                </tbody>
                        ...
                        <div class="card report-table">
                            <div class="card-body">
                                <h4 class="card-title">Recent Student Book Activity (Issues & Returns)</h4>
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <?php if (empty($recentStudentActivity)): ?>
                                                <tr>
                                                    <td colspan="5">No recent student activity to display.</td>
                                                </tr>
                                            <?php else: ?>
                                                <?php foreach ($recentStudentActivity as $activity): ?>
                                                    <tr>
                                                        <td><?php echo htmlspecialchars($activity['issue_id']); ?></td>
                                                        <td><?php echo htmlspecialchars($activity['issue_date']); ?></td>
                                                        <td><?php echo htmlspecialchars($activity['return_date'] ?: 'Not Returned'); ?></td>

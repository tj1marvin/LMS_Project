...
$sort_column = isset($_GET['sort']) ? mysqli_real_escape_string($conn, $_GET['sort']) : 'student_id'; // Default sort column
...
$allowed_columns = ['first_name', 'last_name', 'email', 'approved', 'created_at', 'student_id'];
if (!in_array($sort_column, $allowed_columns)) {
    $sort_column = 'student_id'; // Default back to student_id if invalid
}
...
// Fetch users from the database with search and sort
$sql = "SELECT * FROM students_registration WHERE 1=1"; // 1=1 is always true, makes it easier to add WHERE clauses
  ...
  <div class="wrapper">
    ...
    <div class="page-inner">
        <div class="page-header">
            <h3 class="fw-bold mb-3">View Users</h3>
            <ul class="breadcrumbs mb-3">
                ...
                <li class="nav-item active">
                    <a href="view-students.php" class="nav-link">View Users</a>
                </li>
            </ul>
        </div>
        <div class="row">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <thead class="thead-light">
                                    <tr>
                                        <th>
                                            <a href="?sort=first_name&order=<?php echo ($sort_column == 'first_name' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                                                Name <?php if ($sort_column == 'first_name') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                                            </a>
                                        </th>
                                        <th>
                                            <a href="?sort=email&order=<?php echo ($sort_column == 'email' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                                                Email <?php if ($sort_column == 'email') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                                            </a>
                                        </th>
                                        <th>
                                            <a href="?sort=approved&order=<?php echo ($sort_column == 'approved' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                                                Membership Status <?php if ($sort_column == 'approved') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                                            </a>
                                        </th>
                                        <th>
                                            <a href="?sort=created_at&order=<?php echo ($sort_column == 'created_at' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                                                Joined Date <?php if ($sort_column == 'created_at') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                                            </a>
                                        </th>
                                            ...
                                            echo "<tr>
                                                    ...
                                                    <td class='text-center'>
                                                        <div class='d-inline-block'>
                                                            <a href='view-user.php?id={$row['student_id']}' class='btn btn-info btn-sm'><i class='fas fa-eye'></i> View</a>
                                                        </div>
                                                        <div class='d-inline-block mx-1'>
                                                            <button class='btn btn-warning btn-sm'><i class='fas fa-edit'></i> Edit</button>
                                                        </div>
                                                        <div class='d-inline-block mx-1'>
                                                            <button class='btn btn-danger btn-sm' onclick=\"if(confirm('Are you sure you want to delete this user?')) { window.location.href='delete-user.php?id={$row['student_id']}'; }\"><i class='fas fa-trash-alt'></i> Delete</button>
                                                        </div>
                                                        <div class='d-inline-block'>
                                                            <a href='toggle-status.php?id={$row['student_id']}&status=" . ($row['approved'] == 'active' ? 'inactive' : 'active') . "' class='btn btn-" . ($row['approved'] == 'active' ? 'danger' : 'success') . " btn-sm'><i class='fas fa-power-off'></i> " . ($row['approved'] == 'active' ? 'Deactivate' : 'Activate') . "</a>
                                                        </div>
                                                    </td>
                                                </tr>";
                                        }

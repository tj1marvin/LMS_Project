...
if (isset($_GET['id'])) {
    ...
    // Prepare and execute the SQL statement to fetch book details
    $sql = "SELECT title, author, isbn, genre, year, description, image_path, Available_Copies FROM books WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $book_id);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($title, $author, $isbn, $genre, $year, $description, $image_path, $available_copies);

    if ($stmt->num_rows > 0) {
        $stmt->fetch(); // Fetch the book details
    } else {
        echo "Book not found.";
        exit();
    }
    ...
    $borrowed_sql = "SELECT * FROM issue_book WHERE book_id = ? AND student_id = ? AND status = 'issued'";
    $borrowed_stmt = $conn->prepare($borrowed_sql);
    $borrowed_stmt->bind_param("ii", $book_id, $student_id);
    $borrowed_stmt->execute();
    $borrowed_stmt->store_result();
    $is_borrowed = $borrowed_stmt->num_rows > 0; // Check if the book is borrowed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reserve_book_id'])) {
  $reserve_book_id = intval($_POST['reserve_book_id']);
  $student_id = $_SESSION['student_id']; // Assuming student_id is stored in session
  ...
  // Check if the book is already reserved by the student
  $check_sql = "SELECT * FROM reserved WHERE book_id = ? AND student_id = ?";
  $check_stmt = $conn->prepare($check_sql);
  $check_stmt->bind_param("ii", $reserve_book_id, $student_id);
  $check_stmt->execute();
  ...
  $reserve_sql = "INSERT INTO reserved (student_id, book_id) VALUES (?, ?)";
  $reserve_stmt = $conn->prepare($reserve_sql);
  $reserve_stmt->bind_param("ii", $student_id, $reserve_book_id);

  if ($reserve_stmt->execute()) {
      echo 'success';
  } else {
      echo 'error_' . $conn->error; // Provide detailed error message
  }
...
<body>
  <div class="wrapper">
    ...
    <div class="page-inner">
        <h3 class="fw-bold mb-4">Book Details</h3>

        <div id="book-list" class="row">
            <div class="col-md-6 mb-4">
                <div class="card">
                    <img src="<?php echo htmlspecialchars($image_path); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($title); ?>">
                    <div class="card-body">
                        <h5 class="card-title"><?php echo htmlspecialchars($title); ?></h5>
                        <h6 class="card-subtitle mb-2 text-muted"><?php echo htmlspecialchars($author); ?></h6>
                        <p class="card-text"><strong>ISBN:</strong> <?php echo htmlspecialchars($isbn); ?></p>
                        <p class="card-text"><strong>Genre:</strong> <?php echo htmlspecialchars($genre); ?></p>
                        <p class="card-text"><strong>Publication Year:</strong> <?php echo htmlspecialchars($year); ?></p>
                        <p class="card-text"><strong>Description:</strong> <?php echo nl2br(htmlspecialchars($description)); ?></p>
                        <p class="card-text"><strong>Available Copies:</strong> <?php echo htmlspecialchars($available_copies); ?></p>
                        
                        <?php if ($available_copies > 0): ?>
                            <button class="btn btn-warning borrow-btn" data-book-id="<?php echo $book_id; ?>">
                                Borrow
                            </button>
                        <?php else: ?>
                        ...
                        <button class="btn btn-success reserve-btn" data-book-id="<?php echo $book_id; ?>" <?php echo ($is_borrowed) ? 'disabled' : ''; ?>>
                            Reserve
                        </button>
                        <a href="javascript:void(0);" class="btn btn-secondary" onclick="window.history.back();">Back to Books</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.reserve-btn').click(function() {
            ...
            $.ajax({
                url: '', // This will post to the same page
                type: 'POST',
                data: {
                    reserve_book_id: bookId
                },
                success: function(response) {
                    if (response === 'success') {
                        ...
                        alert('This book is already reserved for you.');
                    } else if (response === 'not_logged_in') {
                        alert('You need to log in to reserve books.');
                        window.location.href = 'Student-login.php'; // Redirect to login page
                    } else {
                        alert('Error reserving book: ' + response);
                    }
                },
                error: function(xhr, status, error) {
                    alert('An error occurred: ' + error);

...
$success_message = "";
$error_message = "";
$overdue_issues = []; // To store overdue book issues for the selected student
...
// Process form submission for adding a fine
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_fine'])) {
    $student_id = isset($_POST['student_id']) ? trim($_POST['student_id']) : '';
    $fine_type = isset($_POST['fine_type']) ? trim($_POST['fine_type']) : '';
    $fine_amount = isset($_POST['fine_amount']) ? trim($_POST['fine_amount']) : '';
    $fine_description = isset($_POST['fine_description']) ? trim($_POST['fine_description']) : '';
    $selected_issue_id = isset($_POST['overdue_issue_id']) ? trim($_POST['overdue_issue_id']) : NULL; // Get selected issue ID

    // Validate inputs (basic validation - add more as needed)
    if (empty($student_id) || !is_numeric($student_id) || empty($fine_type) || empty($fine_amount) || !is_numeric($fine_amount) || $fine_amount <= 0) {
        $error_message = "Please select a Student, Fine Type, and provide a valid Fine Amount.";
    } else if (($fine_type == 'damage' || $fine_type == 'lost') && empty($selected_issue_id)) {
        $error_message = "For 'Damage' or 'Lost' fines, please select an Overdue Book Issue.";
    } else {
        // Check if the student_id exists
        $check_student_sql = "SELECT student_id FROM students_registration WHERE student_id = ?";
        ...
        if ($stmt_check_student->num_rows == 0) {
            ...
            // Use selected issue_id if available
            $issue_id = ($fine_type == 'damage' || $fine_type == 'lost') ? $selected_issue_id : NULL;

            // Check if a fine already exists for this issue_id
            $check_fine_sql = "SELECT fine_id FROM fines WHERE issue_id = ?";
            $stmt_check_fine = $conn->prepare($check_fine_sql);
            $stmt_check_fine->bind_param("i", $issue_id);
            $stmt_check_fine->execute();
            $stmt_check_fine->store_result();

            if ($stmt_check_fine->num_rows > 0) {
                $error_message = "A fine already exists for this book.";
            } else {

                $days_overdue = 0; // You might want to calculate this based on the issue_id
                $status = 'unpaid'; // Default status for new fines

                $insert_fine_sql = "INSERT INTO fines (issue_id, student_id, fine_type, fine_amount, days_overdue, status, description) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt_insert_fine = $conn->prepare($insert_fine_sql);

                if (!$stmt_insert_fine) {
                    $error_message = "Prepare failed: (" . $conn->errno . ") " . $conn->error;
                } else {
                    $stmt_insert_fine->bind_param("iisdiss", $issue_id, $student_id, $fine_type, $fine_amount, $days_overdue, $status, $fine_description);

                    if ($stmt_insert_fine->execute()) {
                        $success_message = "Fine added successfully for Student ID: " . htmlspecialchars($student_id);
                    } else {
                        $error_message = "Error adding fine. Please try again. MySQL Error: " . htmlspecialchars($stmt_insert_fine->error) . " Error Number: " . htmlspecialchars($stmt_insert_fine->errno);
                    }
...
// Fetch overdue books for the selected student.  This part is essential for showing the dropdown.
if (isset($_POST['student_id']) && is_numeric($_POST['student_id'])) {
    $selected_student_id = $_POST['student_id'];
    $sql_overdue = "SELECT ib.issue_id, b.title 
                    FROM issue_book ib 
                    JOIN books b ON ib.book_id = b.id 
                    WHERE ib.student_id = ? 
                    AND ib.status = 'issued' 
                    AND ib.return_date < NOW()"; // Corrected query
    $stmt_overdue = $conn->prepare($sql_overdue);
    $stmt_overdue->bind_param("i", $selected_student_id);
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <div class="card-title">Add New Fine</div>
                            </div>
                            <div class="card-body">
                                ...
                                <form method="POST" action="">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="student_id" name="student_id" required onchange="this.form.submit()">
                                            <option value="">-- Select Student --</option>
                                            <?php foreach ($students as $student): ?>
                                                <option value="<?php echo htmlspecialchars($student['student_id']); ?>" <?php if (isset($_POST['student_id']) && $_POST['student_id'] == $student['student_id']) echo 'selected'; ?>>
                                                    <?php echo htmlspecialchars($student['first_name'] . ' ' . $student['last_name'] . ' (' . $student['enrollment_no'] . ')'); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <small class="form-text text-muted">Select the student to fine.</small>
                                    </div>

                                    <div class="form-group">
                                        <label for="fine_type">Fine Type</label>
                                        <select class="form-control" id="fine_type" name="fine_type" required onchange="this.form.submit()">
                                            <option value="">-- Select Fine Type --</option>
                                            <option value="damage" <?php if (isset($_POST['fine_type']) && $_POST['fine_type'] == 'damage') echo 'selected'; ?>>Damage</option>
                                            <option value="lost" <?php if (isset($_POST['fine_type']) && $_POST['fine_type'] == 'lost') echo 'selected'; ?>>Lost</option>
                                            <option value="other" <?php if (isset($_POST['fine_type']) && $_POST['fine_type'] == 'other') echo 'selected'; ?>>Other</option>
                                        </select>
                                        <small class="form-text text-muted">Specify the type of fine.</small>
                                    </div>

                                    <?php if (isset($_POST['student_id']) && is_numeric($_POST['student_id']) && ($_POST['fine_type'] == 'damage' || $_POST['fine_type'] == 'lost')): ?>
                                        <div class="form-group">
                                            <label for="overdue_issue_id">Select Overdue Book</label>
                                            <select class="form-control" id="overdue_issue_id" name="overdue_issue_id" required>
                                                <option value="">-- Select Book --</option>
                                                <?php foreach ($overdue_issues as $issue): ?>
                                                    <option value="<?php echo htmlspecialchars($issue['issue_id']); ?>">
                                                        <?php echo htmlspecialchars($issue['title']); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                            <small class="form-text text-muted">Select the overdue book for the fine.</small>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" class="form-control" id="fine_amount" name="fine_amount" required placeholder="Enter Fine Amount" step="0.01" min="0.01">
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label for="fine_description">Description (Optional)</label>
                                        <textarea class="form-control" id="fine_description" name="fine_description" rows="3" placeholder="Enter description or reason for the fine"></textarea>

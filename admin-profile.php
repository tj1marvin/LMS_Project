...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['update_password'])) {
        ...
        } elseif (strlen($new_password) < 6) { // Example password strength check
            $error_message = "New password must be at least 6 characters long.";
        } else {
            // Fetch admin's hashed password from the database
            $stmt_select = $conn->prepare("SELECT password FROM admin_login WHERE id = ?");
            $stmt_select->bind_param("i", $admin_id);
            $stmt_select->execute();
            $stmt_select->store_result();

            if ($stmt_select->num_rows > 0) {
                $stmt_select->bind_result($hashed_password_from_db);
                $stmt_select->fetch();
                ...
                if (password_verify($current_password, $hashed_password_from_db)) {
                    ...
                    $stmt_update = $conn->prepare("UPDATE admin_login SET password = ? WHERE id = ?");
                ...
                $error_message = "Admin user not found."; // Should not happen if session is correctly managed
            }
            $stmt_select->close();
        }
    }

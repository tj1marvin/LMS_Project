...
// Function to fetch fines for a student
function getStudentFines($conn, $student_id)
{
  $fines_sql = "SELECT
        f.fine_id,
        ...
        f.status,
        ib.issue_id,
        f.fine_type,  -- Include fine_type in the query
        f.description
    FROM fines f
    LEFT JOIN issue_book ib ON f.issue_id = ib.issue_id
    LEFT JOIN books b ON ib.book_id = b.id
    WHERE f.student_id = ?"; //changed from ib.student_id to f.student_id
...
$fines = getStudentFines($conn, $student_id);

$search_term = isset($_GET['search']) ? $_GET['search'] : '';
$filtered_fines = $fines;

if ($search_term) {
  $filtered_fines = array_filter($fines, function ($fine) use ($search_term) {
    return stripos($fine['book_title'], $search_term) !== false;
  });
}

function compareFines($a, $b, $sort_by, $sort_order)
{
  ...
  switch ($sort_by) {
    ...
    case 'amount':
      return $order * ($a['fine_amount'] - $b['fine_amount']);
    case 'type':
      return $order * strcmp($a['fine_type'], $b['fine_type']);
    case 'status':
      return $order * strcmp($a['status'], $b['status']);
    case 'description':
      return $order * strcmp($a['description'], $b['description']);
    default:
      return 0;
  }
}
...
if ($sort_by) {
  usort($filtered_fines, function ($a, $b) use ($sort_by, $sort_order) {
    return compareFines($a, $b, $sort_by, $sort_order);
  });
}
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
  ...
  if (count($filtered_fines) > 0):
    foreach ($filtered_fines as $fine): ?>
      <tr>
        <td><?php echo htmlspecialchars($fine['book_title']); ?></td>
        <td>
          <span class="badge <?php echo ($fine['status'] == 'paid') ? 'bg-success' : 'bg-danger'; ?> rounded-pill">
            $<?php echo htmlspecialchars($fine['fine_amount']); ?> USD
            <?php if ($fine['days_overdue'] > 0): ?>
              (Overdue: <?php echo htmlspecialchars($fine['days_overdue']); ?> days)
            <?php endif; ?>
        ...
        <td><?php echo ucfirst(htmlspecialchars($fine['status'])); ?></td>
        <td>
          <?php if ($fine['status'] == 'unpaid'): ?>
            <a href="pay-fine.php?fine_id=<?php echo htmlspecialchars($fine['fine_id']); ?>&amount=<?php echo htmlspecialchars($fine['fine_amount']); ?>" class="btn btn-success btn-sm">Pay Now</a>
          <?php endif; ?>
        </td>
      </tr>
    <?php endforeach;
    ...
    <tr>
      <td colspan="6" class="text-center">No fines incurred matching your search.</td>
    </tr>
<?php
  endif;
  $conn->close();
    ...
    <div class="main-panel">
      ...
      <div class="container">
        <div class="page-inner">
          <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
            <div>
              <h3 class="fw-bold mb-3">My Fines</h3>
            </div>
          </div>
          <div class="row">
            <div class="col-md-12">
              <div class="card card-round">
                <div class="card-body">
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title">Fines List</h4>
                    <div class="d-flex">
                      <div class="me-2">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-hover">
                      ...
                      <tbody id="fines-table-body">
                        <?php if (count($filtered_fines) > 0): ?>
                          <?php foreach ($filtered_fines as $fine): ?>
                            <tr>
                              <td><?php echo htmlspecialchars($fine['book_title']); ?></td>
                              <td>
                                <span class="badge <?php echo ($fine['status'] == 'paid') ? 'bg-success' : 'bg-danger'; ?> rounded-pill">
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          const finesTableBody = document.getElementById('fines-table-body'); // Get table body
          ...
          function fetchFines() {
            ...
            fetch('Student-view-fine.php?' + params.toString(), {
                headers: {
              ...
              .then(data => {
                finesTableBody.innerHTML = data; // Update table body with fetched HTML
              })
              .catch(error => {
                console.error('Error fetching fines:', error);
                finesTableBody.innerHTML = '<tr><td colspan="6" class="text-center">Error loading fines.</td></tr>'; //changed colspan to 5, now 6
              });
          }

          sortBySelect.addEventListener('change', fetchFines);
          sortOrderSelect.addEventListener('change', fetchFines);

          searchBox.addEventListener('input', function() {
            fetchFines();
          });

          // Set selected values based on URL parameters (for initial load if needed)

...
$sort_column = isset($_GET['sort']) ? mysqli_real_escape_string($conn, $_GET['sort']) : 'issue_id'; 
$sort_order = isset($_GET['order']) ? mysqli_real_escape_string($conn, $_GET['order']) : 'asc';
$allowed_columns = ['issue_id', 'title', 'student_name', 'issue_date', 'return_date', 'status'];
if (!in_array($sort_column, $allowed_columns)) {
    $sort_column = 'issue_id'; // Default back to issue_id if invalid
}
$sort_order = strtolower($sort_order) == 'desc' ? 'desc' : 'asc'; 
// Base SQL query to fetch issued books with student and book details
$sql = "SELECT ib.issue_id, b.title, CONCAT(s.first_name, ' ', s.last_name) AS student_name, ib.issue_date, ib.return_date, ib.status
        FROM issue_book ib
        JOIN books b ON ib.book_id = b.id
        JOIN students_registration s ON ib.student_id = s.student_id WHERE 1=1"; // 1=1 for easy condition appending
// Search condition
if (!empty($search_term)) {
    $sql .= " AND (b.title LIKE '%{$search_term}%' OR CONCAT(s.first_name, ' ', s.last_name) LIKE '%{$search_term}%')";
}
// Sorting
      ...
      <div class="container">
        <div class="page-inner">
          <div class="page-header">
            <h3 class="fw-bold mb-3">Issued Books</h3>
            <ul class="breadcrumbs mb-3">
              ...
              <li class="nav-item">
                <a href="view-issued-book.php">View Issued Books</a>
              </li>
            </ul>
          </div>
          <div class="row">
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="search-box">
                      <form method="GET" action="">
                        <div class="input-group">
                          <input type="text" class="form-control" placeholder="Search Books or Student" name="search" value="<?php echo htmlspecialchars($search_term); ?>">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped">
                      <thead>
                        <tr>
                          <th>
                            <a href="?sort=issue_id&order=<?php echo ($sort_column == 'issue_id' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                              Issue ID <?php if ($sort_column == 'issue_id') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                            </a>
                          </th>
                          <th>
                            <a href="?sort=title&order=<?php echo ($sort_column == 'title' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                              Book Title <?php if ($sort_column == 'title') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                            </a>
                          </th>
                          <th>
                            <a href="?sort=student_name&order=<?php echo ($sort_column == 'student_name' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                              Student Name <?php if ($sort_column == 'student_name') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                            </a>
                          </th>
                          <th>
                            <a href="?sort=issue_date&order=<?php echo ($sort_column == 'issue_date' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                              Issue Date <?php if ($sort_column == 'issue_date') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                            </a>
                          </th>
                          <th>
                            <a href="?sort=return_date&order=<?php echo ($sort_column == 'return_date' && $sort_order == 'asc') ? 'desc' : 'asc'; ?>">
                              Return Date <?php if ($sort_column == 'return_date') echo ($sort_order == 'asc' ? '&#9650;' : '&#9660;'); ?>
                            </a>
                          </th>
                          <th>
                        ...
                        if ($result->num_rows > 0) {
                          ...
                          while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                                ...
                                                <td>" . ($row['return_date'] ? date('d M Y', strtotime($row['return_date'])) : 'Not Returned') . "</td>
                                                <td>{$row['status']}</td>
                                            </tr>";
                          }
                        } else {

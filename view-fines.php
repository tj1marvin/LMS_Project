...
$all_fines_sql_with_limit = $all_fines_sql . " LIMIT $start, $limit";
$all_fines_result_with_limit = $conn->query($all_fines_sql_with_limit);

// Get total number of records for pagination
$total_records_sql = "SELECT COUNT(*) FROM fines";
...
function getSortOrder($column) {
    return isset($_GET['sort']) && $_GET['sort'] === $column ? (isset($_GET['order']) && $_GET['order'] === 'asc' ? 'desc' : 'asc') : 'asc';
}

$sort = isset($_GET['sort']) ? $_GET['sort'] : 'student';  // Default sort column
$order = isset($_GET['order']) ? $_GET['order'] : 'asc';  // Default sort order

switch ($sort) {
    case 'student':
        $all_fines_sql .= " ORDER BY s.first_name $order, s.last_name $order";
        break;
    case 'fine_type':
        $all_fines_sql .= " ORDER BY f.fine_type $order";
        break;
    case 'amount':
        $all_fines_sql .= " ORDER BY f.fine_amount $order";
        break;
    case 'status':
        $all_fines_sql .= " ORDER BY f.status $order";
        break;
    default:
        $all_fines_sql .= " ORDER BY s.first_name $order, s.last_name $order";
}
...
<div class="container">
    <div class="page-inner">
        <div class="page-header">
            ...
            <ul class="breadcrumbs mb-3">
                ...
                <li class="nav-item">
                    <a href="view-fines.php">Fines List</a>
                </li>
            </ul>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">List of Fines</div>
                        <div class="row">
                            <div class="col-md-4">
                                <label for="showEntries" class="mr-2">Show Entries:</label>
                                <select id="showEntries" class="form-control-sm">
                            ...
                            <div class="col-md-8">
                                <input type="text" id="searchInput" class="form-control" placeholder="Search Student, Fine Type, Status, Description...">
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <?php if ($all_fines_result->num_rows > 0): ?>
                            <div class="table-responsive">
                                <table class="table table-striped mt-3" id="all-fines-table">
                                    <thead>
                                        <tr>
                                            <th onclick="sortTable(0)" style="cursor: pointer;">
                                                Student <a href="?page=<?php echo $page; ?>&sort=student&order=<?php echo getSortOrder('student'); ?>"><i class="fas fa-sort"></i></a>
                                            </th>
                                            <th onclick="sortTable(1)" style="cursor: pointer;">
                                                Fine Type <a href="?page=<?php echo $page; ?>&sort=fine_type&order=<?php echo getSortOrder('fine_type'); ?>"><i class="fas fa-sort"></i></a>
                                            </th>
                                            <th>Book Title (if overdue)</th>
                                            <th onclick="sortTable(2)" style="cursor: pointer;">
                                                Fine Amount <a href="?page=<?php echo $page; ?>&sort=amount&order=<?php echo getSortOrder('amount'); ?>"><i class="fas fa-sort"></i></a>
                                            </th>
                                            <th>Description</th>
                                            <th onclick="sortTable(3)" style="cursor: pointer;">
                                                Status <a href="?page=<?php echo $page; ?>&sort=status&order=<?php echo getSortOrder('status'); ?>"><i class="fas fa-sort"></i></a>
                                            </th>
                                            <th>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php while ($fine = $all_fines_result_with_limit->fetch_assoc()): ?>
                                            <tr>
                                                <td>
                                                    <a href="view-student-details.php?student_id=<?php echo htmlspecialchars($fine['student_id']); ?>" target="_blank">
                                                        <?php echo htmlspecialchars($fine['first_name'] . ' ' . $fine['last_name'] . ' (' . $fine['enrollment_no'] . ')'); ?>
                                                    </a>
                                                </td>
                                                <td><?php echo htmlspecialchars(ucfirst($fine['fine_type'])); ?></td>
                                                <td><?php echo isset($fine['book_title']) ? htmlspecialchars($fine['book_title']) : 'N/A'; ?></td>
                                                <td>$<?php echo htmlspecialchars(number_format($fine['fine_amount'], 2)); ?></td>
                                                <td><?php echo htmlspecialchars($fine['fine_description'] ?? 'N/A'); ?></td>
                                                <td>
                                                    <?php
                                                    $status_class = '';
                                                    if ($fine['fine_status'] == 'paid') {
                                                        $status_class = 'status-paid';
                                                    } else if ($fine['fine_status'] == 'unpaid') {
                                                        $status_class = 'status-unpaid';
                                                    }
                                                    echo '<span class="' . $status_class . '">' . htmlspecialchars(ucfirst($fine['fine_status'])) . '</span>';
                                                    ?>
                                                </td>
                                                <td class="action-icons">
                                                    <a href="edit-fine.php?fine_id=<?php echo htmlspecialchars($fine['fine_id']); ?>" title="Edit Fine"><i class="fas fa-edit"></i></a>
                                                    <button class="delete-fine btn btn-link text-danger" data-id="<?php echo htmlspecialchars($fine['fine_id']); ?>" title="Delete Fine"><i class="fas fa-trash"></i></button>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    </tbody>
                            ...
                            <div class="pagination-container mt-3">
                                <ul class="pagination">
                                    <li class="page-item <?php echo ($page == 1) ? 'disabled' : ''; ?>">
                                        <a class="page-link" href="?page=<?php echo $page - 1; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?>" aria-label="Previous">
                                    ...
                                    <li class="page-item <?php echo ($page == $total_pages) ? 'disabled' : ''; ?>">
                                        <a class="page-link" href="?page=<?php echo $page + 1; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?>" aria-label="Next">
    ...
    <script>
        function sortTable(columnIndex) {
            ...
            table = document.getElementById("all-fines-table"); // Updated table ID
            switching = true;
            ...
            button.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Use fetch API to send asynchronous request
                        fetch('delete_fine.php', {  // Corrected file name to delete_fine.php
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
                                body: 'fine_id=' + encodeURIComponent(fineId), // Corrected parameter name to fine_id
                            })
                            .then(response => {
                            ...
                            .then(data => {
                                // Handle the response from delete_fine.php
                                if (data.status === 'success') {
                                    Swal.fire({
                                        ...
                                        window.location.href = "view-fines.php"; // Redirect after SweetAlert confirmation
                                    });
                                } else {
                                    Swal.fire({
                                        title: "Error!",
                            ...
                            .catch(error => {
                                ...
                                Swal.fire({
                                    title: "Error!",
                                    text: "Failed to delete the fine: " + error,
                                    icon: "error",
                                });
        ...
        // сохраняем значение searchInput в query parameter
        ...
        // восстанавливаем значение searchInput из query parameter
        window.onload = function() {
            ...
            if (limitParam) {
                document.getElementById('showEntries').value = limitParam;
            }
        }
    </script>

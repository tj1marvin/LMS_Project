...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $days = intval($_POST['days']); // Check if the book has available copies
    $check_availability_sql = "SELECT Available_Copies FROM books WHERE id = ?";
    $check_availability_stmt = $conn->prepare($check_availability_sql);
    $check_availability_stmt->bind_param("i", $book_id);
    $check_availability_stmt->execute();
    $check_availability_stmt->bind_result($available_copies);
    $check_availability_stmt->fetch();
    $check_availability_stmt->close();
    if ($available_copies > 0) {// Calculate the return date
        $return_date = date('Y-m-d H:i:s', strtotime("+$days days")); // Insert into issue_book table
        $sql = "INSERT INTO issue_book (book_id, student_id, issue_date, return_date, status) VALUES (?, ?, NOW(), ?, 'issued')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iis", $book_id, $student_id, $return_date);
        if ($stmt->execute()) {   // Decrease the available copies
            ...
            if ($update_copies_stmt->execute()) {
                echo "<script>alert('Book issued successfully!'); window.location.href='view-issued-book.php';</script>";
            } else {
                echo "Error updating book availability: " . $update_copies_stmt->error;
            }
        ...
        // Handle book request if it's not available
        $sql_request = "INSERT INTO book_requests (student_id, book_id, request_date, status) VALUES (?, ?, NOW(), 'pending')";
        $stmt_request = $conn->prepare($sql_request);
        $stmt_request->bind_param("ii", $student_id, $book_id);
        if ($stmt_request->execute()) {
            echo "<script>alert('Book request sent successfully!'); window.location.href='view-book-requests.php';</script>";
        } else {
            echo "Error sending book request: " . $stmt_request->error;
        }
...
// Fetch students
$students_sql = "SELECT * FROM students_registration WHERE approved = 'active'";
  ...
  <div class="wrapper">
    ...
    <div class="page-inner">
        <div class="page-header">
            <h3 class="fw-bold mb-3">Book Requests</h3>
            <ul class="breadcrumbs mb-3">
                ...
                <li class="nav-item">
                    <a href="book-requests.php">Book Requests</a>
                </li>
            </ul>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Requested Books</div>
                    </div>
                    <div class="card-body">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                $query = "SELECT br.request_id, sr.first_name, sr.last_name, b.title, br.request_date, br.status 
                                          FROM book_requests br 
                                          JOIN students_registration sr ON br.student_id = sr.student_id 
                                ...
                                if ($result->num_rows > 0) {
                                    while ($request = $result->fetch_assoc()) {
                                        ...
                                        if ($request['status'] == 'pending') {
                                            $status_btn = '<a href="approve_request.php?id=' . $request['request_id'] . '" class="btn btn-sm btn-danger">Approve</a>';
                                        } else {
                                            $status_btn = '<span class="badge badge-success">Approved</span>';
                                        }
                                        echo "<tr>

...
/**
 * Function to Get Most Popular Books Report (SECURE - Prepared Statement)
 * Fetches a list of the most frequently issued books.
 * @param mysqli $conn Database connection object.
 * @param int $limit Maximum number of books to retrieve (default 10).
 * @return array Array of popular books data, or empty array on error.
 */
// Function to get most popular books report
function getMostPopularBooksReport($conn, $limit = 10)
{
    $sql = "SELECT
                b.id AS book_id,
            ...
            FROM books b
            LEFT JOIN issue_book ib ON b.id = ib.book_id
    ...
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        error_log("getMostPopularBooksReport Prepare Error: " . $conn->error);
        return [];
    }

    $stmt->bind_param("i", $limit);
    if (!$stmt->execute()) {
        error_log("getMostPopularBooksReport Execute Error: " . $stmt->error);
        return [];
    }
...
// --- Fetch Report Data ---
$popularBooksReport = getMostPopularBooksReport($conn);

// --- Fetch Recent Messages and Notifications (as in your code - included for completeness, even if not displayed in current HTML) ---
            ...
            <div class="content">
                <div class="container-fluid">
                    <div class="report-container">
                        ...
                        <h1 class="mb-4">Book Usage Report</h1>

                        <div <h2>
                            <p class="mb-3">This report shows the most popular books in the library based on the number of times they have been issued.</p>
                            </h2>
                        </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            <?php if (empty($popularBooksReport)): ?>
                                                <tr>
                                                    <td colspan="4">No book usage data available.</td>
                                                </tr>
                                            <?php else: ?>
                                                <?php $rank = 1;
                                                foreach ($popularBooksReport as $book): ?>
                                                    <tr>
                                                        <td><?php echo $rank++; ?></td>
